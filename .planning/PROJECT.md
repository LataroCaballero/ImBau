...
| Worker email: externalizar `react`/`react-dom`/`react-dom/server` en `tsup` + `react-dom` como dep directa del worker (Phase 11 / G-11-3) | El bundle ESM único del worker inlineaba `react-dom/server` (CJS, usado por @react-email/render); su `require("react")` interno rompe con "Dynamic require of react"; externo carga como CJS desde node_modules — mismo patrón ya documentado para pino. react-dom era solo transitivo → hay que declararlo directo para que pnpm lo linkee | ✓ Good — descubierto en UAT de Phase 11 (el envío real nunca se había ejercido; verificación solo tocaba el fallback dev-console); fix verificado E2E, mail entregado a la casilla configurada |
