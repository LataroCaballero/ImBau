...
- **D-14:** Todo cambio de precio/estado (inline, bulk, o import aplicado) dispara **revalidación ISR on-demand** del picker/cotizador público al instante. El mecanismo exacto (`revalidateTag`/`revalidatePath` + tags por proyecto/slug) queda a **research + planning** (ver research flag). Requisito de producto: **instantáneo**, sin esperar TTL.
